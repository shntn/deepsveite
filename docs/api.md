...
* `build`
  ...
  続けて、SystemVerilog の `always_comb` と同様に、時刻 0 ですべての `always_comb` を 1 回評価して収束させる。
  そのため、多重ドライバや収束しない組み合わせループは、この `build` の時点でエラーになることがある。
  `VCD.new` の前に呼び出す。
...
`sim.build` の最後（時刻 0）に、すべての `always_comb` を 1 回評価して収束させる。
最初のクロックエッジで `always_ff` が読む `always_comb` の出力も、初期評価済みの値になる。
